...
export class WarrantyCenterController {
  ...
  async getAllWarranty(@AccountDecorator() account: AccountDocument, @Query() warrantyQuery: WarrantyQueryDto, @Query() options: PaginationParamsDto) {
    if (account.roles.includes(Role.DistributionAgent) && account.roles.includes(Role.WarrantyCenter)) {
      throw new HttpException("You must have role: DistributionAgent or WarrantyCenter", HttpStatus.BAD_REQUEST);
    }
    if (account.roles.includes(Role.DistributionAgent)) {
      const distributionAgent = await this.distributionAgentService.findOne({ _id: account.belongTo });
      const {
        data,
        paginationOptions
      } = await this.warrantyService.findAll({ fromDistributionAgent: distributionAgent, ...warrantyQuery }, {
